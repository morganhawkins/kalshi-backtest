...
class SimKalshiMarket(BaseMarket):
    ...
    def _fill_order(self, order: BaseOrder, price: Number) -> None:
        ...
        print(f"filled at (tte:{self.tte/3600}) (price: ${fill_price}) ", order)
    ...
    def limit_order(self, contracts: int, side: Literal["buy", "sell"], price: Number) -> None:
        ...
        if (side=="buy") and (price >= market['ask']):
            ...
            self._orders.append(LimitOrder(count=contracts, side=side, price=price))
    ...
    def clear_orders(self) -> None:
        self._orders = [order for order in self._orders if isinstance(order, MarketOrder)]

    def remove_orders(self, side=None, price=None) -> None:
        orders_left = [order for order in self._orders if ((order.side!=side) and (order.price!=price) and not isinstance(order, MarketOrder))]
        self._orders = orders_left
